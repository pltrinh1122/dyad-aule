#!/usr/bin/env bash
# Acceptance criteria for bin/d-reconcile — the safe start-of-session reconcile.
...
# drives the REAL bin/d-reconcile against it, asserting the safety envelope holds. Nothing here touches
# the actual dyad-aule repo — every scenario runs inside its own mktemp fixture with cwd set to it.
here="$(cd "$(dirname "$0")" && pwd)"; repo="$(cd "$here/.." && pwd)"
source "$here/_lib.sh"
RECON="$repo/bin/d-reconcile"
...
assert "committed executable (git 100755): bin/d-reconcile" \
  bash -c '[ "$(cd "'"$repo"'" && git ls-files -s bin/d-reconcile | cut -d" " -f1)" = "100755" ]'
