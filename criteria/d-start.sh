...
# Real-half only. Exercises d-start with DYAD_DSTART_SKIP_CHECK=1 so it does NOT re-enter ./check
# (this file runs UNDER ./check; letting d-start call ./check here would recurse forever).
...
out="$(DYAD_DSTART_SKIP_CHECK=1 "$ds" 2>&1 || true)"
has() { printf '%s\n' "$out" | grep -q "$1"; }
...
DYAD_DSTART_SKIP_CHECK=1 "$ds" >/dev/null 2>&1 || true
