...
| *(gh-issues)* | queue | the activity queue — **nodes live as GitHub issues** (issues-as-home, N11): one issue = one node (body = intent + `Depends-on:` + `Done:`, exactly one `status:*` lane). Lifecycle **status is stored** and **guarded** by the FSM (`criteria/state-guard.sh`); schema by `criteria/issue-node-schema.sh`. The decision record: `dialectic/2026-07-08-plan-home-local-vs-issues.md`. |
| `.dyad-state/` | state | state capture for resume (SPAOR **Observe**) — the op-provenance carve-out: a commit touching **only** this home is state-capture, exempt from `Node:` lineage. Home of the **plan-cache** (`.dyad-state/plan-cache/`, N15): the offline, check-reachable projection of the issue-truth that `bin/_frontier` reads and `Node: #<issue>` provenance resolves against. |
